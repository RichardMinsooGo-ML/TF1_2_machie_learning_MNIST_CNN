...
H_SIZE_04 = 258
N_EPISODES = 10
# 100, 200, 500, 1000, 2000, 5000. For MNIST 5000 might work. If memory is insufficient choose small value
batch_size = 5000
...
W_output = tf.get_variable("W_output", shape=[H_SIZE_04, N_Classes],
                           initializer=tf.contrib.layers.xavier_initializer())
...
LAY01_m = tf.nn.dropout(_LAY01_m, keep_prob=keep_prob)
_LAY02_m = tf.nn.relu(tf.matmul(LAY01_m, W02_m) + B02_m)
LAY02_m = tf.nn.dropout(_LAY02_m, keep_prob=keep_prob)
_LAY03_m = tf.nn.relu(tf.matmul(LAY02_m, W03_m) + B03_m)
LAY03_m = tf.nn.dropout(_LAY03_m, keep_prob=keep_prob)
_LAY04_m = tf.nn.relu(tf.matmul(LAY03_m, W04_m) + B04_m)
LAY04_m = tf.nn.dropout(_LAY04_m, keep_prob=keep_prob)
Pred_m = tf.matmul(LAY04_m, W_output) + B_output
