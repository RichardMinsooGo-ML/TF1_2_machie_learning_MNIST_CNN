...
N_Input = 784
N_Classes = 10
# H_SIZE_01 = 256  : it is not used because there are no layers
# H_SIZE_02 = 255  : it is not used because there are no layers
N_EPISODES = 10
...
W_output = tf.Variable(tf.random_normal([N_Input, N_Classes]))
B_output = tf.Variable(tf.random_normal([N_Classes]))

# Step 6. Define the model structure:
Pred_m = tf.matmul(X, W_output) + B_output
