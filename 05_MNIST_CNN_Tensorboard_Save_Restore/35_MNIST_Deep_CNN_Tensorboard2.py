...
DIR_Checkpoint  = "/tmp/ML/09_CNN_Tensorboard_Save_Restore/CheckPoint"
DIR_Tensorboard = "/tmp/ML/09_CNN_Tensorboard_Save_Restore/Tensorboard"
...
with tf.name_scope('Hidden_Layer_01'):
    ...
    tf.summary.histogram("X_Image", X_img)
    tf.summary.histogram("Weights_01", W01_m)
...
with tf.name_scope('Hidden_Layer_02'):
    ...
    tf.summary.histogram("Weights_02", W02_m)
...
with tf.name_scope('Hidden_Layer_03'):
    ...
    tf.summary.histogram("Weights_03", W03_m)
...
with tf.name_scope('Hidden_Layer_04'):
    ...
    tf.summary.histogram("Weights_04", W04_m)
...
with tf.name_scope('Output_Layer'):
    ...
    tf.summary.histogram("Weights_05", W05_m)
    tf.summary.histogram("Output", Pred_m)
    
with tf.name_scope('Optimizer'):
    # define cost/loss & optimizer
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=Pred_m, labels=Y))
    optimizer = tf.train.AdamOptimizer(learning_rate = Alpha_Lr).minimize(cost, global_step=global_step)
